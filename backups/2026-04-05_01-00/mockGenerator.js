...
const GENRE_ACTIONS = {
  ...
  Culture: [
    "Scene 1: Aerial Introduction — Panoramic sweep of the Swiss Alps with macro dew and wildflowers",
    "Scene 2: Rajagopal Kannan Intro — Indian hero practicing martial arts in a sunlit meadow",
    "Scene 3: Japanese Heroine Intro — Performing a traditional dance near a tranquil lake",
    "Scene 4: The Meeting — Rajagopal and Japanese heroine meet in a picturesque flower valley",
    "Scene 5: Cultural Exchange — Sharing sushi and samosas, teaching each other dance moves",
    "Scene 6: Journey — Rajagopal and Japanese heroine explore the Swiss landscape together",
    "Scene 7: Climactic Viewpoint — Silhouettes against the Alps, Rajagopal and Japanese heroine in unity",
    "Scene 8: Lake Reflection — Rajagopal and Japanese heroine sharing stories and laughter",
    "Scene 9: Closing Panoramic Shot — Sunset over the Alps, vibrant colors reflecting their bond",
  ],
  Wonders: [
    "Scene 1: Great Wall — Rajagopal spins the Japanese heroine in a pivot dance under the moonlight, identity preserved in the cinematic night air",
    "Scene 2: Taj Mahal — Close-up on Rajagopal and the Japanese heroine embracing as dawn light glows behind them, pixel-perfect identity synthesis",
    "Scene 3: Machu Picchu — High altitude waltz on the terraces, Rajagopal and Japanese heroine silhouette against the ancient ruins",
    "Scene 4: Petra — Torchlit dance by the Treasury, Rajagopal face integrated into the golden torchlight glow as he gazes at the heroine",
    "Scene 5: Colosseum — Roman night serenade, Rajagopal supports the Japanese heroine in a dip dance under cinematic spotlights",
    "Scene 6: Rio de Janeiro — Sunset finale at Christ the Redeemer, Rajagopal and Japanese heroine stand together in a triumphant cinematic pose",
    "Scene 7: Chichen Itza — Moonlight dance sequence, Rajagopal and Japanese heroine glide across the ancient plaza with firefly VFX",
    "Scene 8: Great Pyramid — Tender Giza sunrise, Rajagopal identity locked onto the hero as he shields the heroine from the morning sun",
    "Scene 9: Angkor Wat — Sacred reflection dance, Rajagopal and Japanese heroine perform a slow-motion finale by the lotus pond",
  ],
};
...
const extractKeywords = (text) => {
  ...
  if (text.match(/harmony|culture|swiss|india|japan|sitar|shakuhachi/i)) keywords.push('Harmony of Cultures', 'Cinematic Song Sequence');
  if (text.match(/wonder|wall of china|taj mahal|machu picchu|petra|colosseum|brazil|mexico|egypt|cambodia/i)) keywords.push('9 Wonders of the World', 'Global Romance');
...
export const parsePromptToMockJSON = (ideaText, duration, genre, includeHero, heroName = 'Lead Protagonist', aspectRatio = '16:9', engine = 'GAN (Adversarial)') => {
  ...
  const isWonders = text.includes('wonder') || text.includes('taj') || text.includes('pyramid') || text.includes('china');
  const isCulture = text.includes('harmony') || text.includes('culture') || text.includes('swiss') || text.includes('sitar');
  const keywords = extractKeywords(ideaText);
  const numScenes = (isWonders || isCulture) ? 9 : (text.includes('predator') ? 10 : (duration >= 3 ? 3 : duration >= 2 ? 2 : 1));
  const durLabel = duration === 1 ? '1 minute' : `${duration} minutes`;
  const actions = isWonders ? GENRE_ACTIONS.Wonders : (isCulture ? GENRE_ACTIONS.Culture : (GENRE_ACTIONS[genre] || GENRE_ACTIONS['Action']));
